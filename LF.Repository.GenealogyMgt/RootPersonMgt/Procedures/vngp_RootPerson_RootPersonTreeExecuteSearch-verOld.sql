...
BEGIN
		...
		IF @BranchId <>''
		BEGIN
			SET @strBranch = N' AND RP.RootBranchId = @BranchId ' 
			SET @strBranch1 = N' AND Rs.RootBranchId = @BranchId ' 
		END

		IF @PersonId <>''
		BEGIN
			SET @strPerson = N' AND RP.RootPersonId = @PersonId '
		END
	...
	DECLARE @declareParams NVARCHAR(1000) = '
		@BranchId INT,
		@PersonId INT
	';
	EXECUTE sp_executesql @query, @declareParams, @BranchId = @BranchId, @PersonId = @PersonId
	SELECT(@query)
END